package eVoting.rmiserver;// Existe um Multicast Server por cada mesa de voto que gerelocalmente os terminais de voto que lhe estão associados.  Permite aos membros da mesa realizar a funcionalidade 6 e realiza automaticamente a funcionalidade 5
...
import java.rmi.*;
import eVoting.protocol.Protocol;
    ...
    public static void main(String[] args) {
        ...
        Runtime.getRuntime().addShutdownHook(new Thread() {
            public void run() {
                try {
                    Thread.sleep(200);
                    System.out.println("Shutting down ...");  // mensagem de saída
                    if (q.getSearchingTerminal()!=null) {  // se houver algum que esteja fora 
            ...
            while (true) {
                ...
                if (option == 0) {
                    ...
                    if((voter!=null && voter.department.equalsIgnoreCase(q.getDepartment()))){
                        // HERE
                        if (q.votingContains(voter)==null && q.requestContains(voter)==null && (q.getSearchingTerminal()==null || (q.getSearchingTerminal()!=null && !q.getSearchingTerminal().getUsername().equalsIgnoreCase(voter.getUsername()))))
