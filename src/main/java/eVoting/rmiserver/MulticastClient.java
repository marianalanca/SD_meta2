                                            ...
                                            if (password.equals(data.getPassword())) {
                                                ...
                                                if (protocol.item_count==0) {
                                                    ...
                                                    if (protocol.item_count==0){ 
                                                        ...
                                                        // colocar timeout e vai ficando em espera enquanto não receber o que pretende; se chegar ao catch, fica num while a tentar enviar até receber
                                                        // HERE
                                                        try{
                                                            socket.setSoTimeout(3000); // waits for 3 seconds
                                                            socketResult.send(packet);
                                                            do {
                                                                buffer = new byte[256];
                                                                packet = new DatagramPacket(buffer, buffer.length);
